  ...
  <SectionShell ref={ref} index={index}>
    <div>
      ...
      <div className={cn("rounded-2xl border border-border/40 bg-card/80 p-5 mb-5", anim(index))} style={{ transitionDelay: "180ms" }}>
        <p className="font-mono text-xs tracking-widest text-muted-foreground uppercase mb-3">5가지 실행 모드</p>
        <div className="flex flex-wrap gap-2">
          {["autopilot", "ralph", "ultrawork (ulw)", "/team N:executor", "ccg"].map((mode) => (
            <span key={mode} className="font-mono text-xs text-primary bg-primary/10 px-3 py-1.5 rounded-lg">{mode}</span>
          ))}
        </div>
      </div>
      ...
      <div className={cn("grid grid-cols-3 gap-4", anim(index))} style={{ transitionDelay: "260ms" }}>
        ...
        <div className="rounded-2xl border border-border/40 bg-card/80 p-5 shadow-sm backdrop-blur-sm">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            ...
            <span className="font-mono text-xs text-primary">node -v</span>로 확인.
          </p>
        </div>
        <div className="rounded-2xl border border-border/40 bg-card/80 p-5 shadow-sm backdrop-blur-sm">
